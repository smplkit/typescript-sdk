...
function wrapFetchError(err: unknown): never {
  ...
  if (err instanceof DOMException && err.name === "AbortError") {
    throw new SmplTimeoutError(`Request timed out`);
  }
...
export class ConfigClient {
  ...
  async list(): Promise<Config[]> {
    ...
    try {
      ...
      if (result.error !== undefined) {
        await checkError(result.response, "Failed to list configs");
      }
      data = result.data;
    } catch (err) {
      wrapFetchError(err);
  ...
  async create(options: CreateConfigOptions): Promise<Config> {
    ...
    try {
      ...
      if (result.error !== undefined) {
        await checkError(result.response, "Failed to create config");
      }
      data = result.data;
    } catch (err) {
      wrapFetchError(err);
  ...
  async _updateConfig(payload: ConfigUpdatePayload): Promise<Config> {
    ...
    if (!data) throw new SmplValidationError(`Failed to update config ${payload.configId}`);
    return resourceToConfig(data.data, this);
  }
